...
w = 0.01:0.01:1000;%w dla modelu ciąłego


wn =2*fNY*pi; %w dla częstotliwości Nyquista
wd = 0.01:0.01:2*fNY*pi;

%Obliczenie wartości modułów oraz faz dla modelu ciągłego
X = arrayfun(f,w);
mod = abs(X);
amp = 20*log10(mod);
arg = angle(X);
...
hold on
grid on 
semilogx(wd,ampd,'black','LineWidth' , 2.0)%dla modelu dyksretnego
hold on
xlim([10.^-2,10.^3])
